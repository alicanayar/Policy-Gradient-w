...
class ValueCritic(nn.Module):
    ...
    def update(self, obs: np.ndarray, q_values: np.ndarray) -> dict:
        ...
        optimizer.zero_grad()
        q_predict = self.forward(obs)
        q_predict = q_predict.squeeze(1)
        print("q predict",q_predict.shape, q_predict)
        print("q values",q_values.shape, q_values)
        loss_mse = nn.MSELoss()
        loss = loss_mse(q_predict,q_values)
